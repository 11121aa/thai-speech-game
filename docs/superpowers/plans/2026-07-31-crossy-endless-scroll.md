# Crossy Road Endless Forward Scroll Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace `js/game-crossy.js`'s fixed 7-row board (which teleports the frog back to the start on both goal-reached and car-hit) with an endless, camera-scrolled world that never resets position.

**Architecture:** The frog's logical row (`worldRow`) becomes an unbounded integer instead of a 0-6 board index. A pure function `rowDefAt(worldRow)` derives each row's type/direction/speed from `worldRow mod GOAL_SPACING`, reproducing the original road/road/grass/road/road rhythm indefinitely. A lazily-populated row registry (`this.rows`, keyed by worldRow) holds each generated row's cars; rows are generated just ahead of the visible window and discarded once they scroll well behind it. The frog is drawn at a fixed screen position (`ANCHOR_ROW_SLOT`); a tweened `scrollWorldRow` camera value — driven by the exact same tween call that already animates the frog's column hop — makes the terrain slide smoothly past it.

**Tech Stack:** Phaser 3 (Scene, Graphics, Tweens), vanilla JS. No build step, no test framework — this file has no automated tests today (confirmed: no `package.json` / test runner anywhere in the repo) and none are being introduced; verification is manual playtesting through `game.html`'s normal entry point, per the spec's own Testing section.

## Global Constraints

- Only `js/game-crossy.js` is touched — there is no standalone dev build for this game (unlike the platformer's `platformer-standalone.html`), so no dual-file update applies here.
- No change to sideways (column) movement, the 11-column width, car visuals, frog visuals, or the practice modal itself (spec Non-goals).
- No game-over-on-hit — hits knock back, they never end the run (spec Non-goals).
- `GOAL_SPACING = 6`, `KNOCKBACK_ROWS = 2` — exact values from the approved spec (`docs/superpowers/specs/2026-07-31-crossy-endless-scroll-design.md`).

---

### Task 1: Rewrite `js/game-crossy.js` for endless forward scroll

**Files:**
- Modify: `js/game-crossy.js` (full-file rewrite — nearly every function touches the row/camera state, so an incremental edit would leave the file in a non-running state between edits; a single complete replacement is the reviewable, bisectable unit here)

**Interfaces:**
- Consumes: `Phaser` (global, already loaded by `game.html` before this script), `words` and `callbacks` parameters exactly as `createCrossyGame(words, callbacks)` already receives them today — no signature change. `callbacks.onPoints`, `callbacks.onPractice`, `callbacks.onTime` are called with the same argument shapes as today.
- Produces: `CrossyGame.start(words, cbs)` / `CrossyGame.stop()` — the public API `game.html` calls stays byte-for-byte identical; nothing outside this file needs to change.

- [ ] **Step 1: Replace the full contents of `js/game-crossy.js`**

Write the complete file below, replacing everything currently in `js/game-crossy.js`:
...
Open `game.html` in a browser, start the crossy-road ("ข้ามถนน") game with a word category selected, and verify each of these:

1. **Initial layout matches today**: at spawn, the row layout top-to-bottom should look identical to the current game (goal strip at top, then road/road/grass/road/road, frog near the bottom) — confirms `rowDefAt()` reproduces the original rhythm.
2. **Smooth forward scroll**: press ↑ repeatedly. The frog should stay near the same screen position while the terrain visibly slides downward each hop (not an instant jump) — confirms the shared tween is animating `scrollWorldRow` correctly.
3. **No reset on goal**: cross to the goal row, complete the word-practice modal. After it closes, the frog should still be standing on the same row/column it reached — not teleported anywhere. Continue pressing ↑ past that point — new terrain should keep generating with no gap or crash.
4. **Checkpoint spacing**: count hops between two consecutive goal rows — should be exactly `GOAL_SPACING` (6).
5. **Hit knockback, not reset**: deliberately walk into a car. Confirm: -5s timer penalty fires, a brief invincibility flicker plays, and the frog moves backward exactly `KNOCKBACK_ROWS` (2) rows — not all the way back to the original start position.
6. **Backward movement boundary**: press ↓ repeatedly near the bottom of the currently generated window — should stop cleanly at the boundary, no error thrown, no visual glitch.
7. **Sideways movement unaffected**: ← and → should feel exactly as before (instant-feeling tween, blocked at column 0/10 edges).
8. **Extended play, no leak**: play through at least 4-5 checkpoints continuously. No slowdown, no visual popping-in of terrain, no console errors.
9. **Console check**: open browser devtools console during the whole session above — zero errors or warnings from `game-crossy.js`.

- [ ] **Step 3: Commit**

```bash
git add js/game-crossy.js
git commit -m "$(cat <<'EOF'
Replace crossy-road's fixed board with endless forward scroll

Reaching a goal or getting hit by a car no longer teleports the frog
back to a fixed start position. The board is now generated endlessly
as the frog advances, with the camera scrolling smoothly instead of
resetting, and a hit knocks the frog back a few rows instead of all
...
- **Spec coverage:** Section 0 (coordinate system) → Task 1's `worldRow`/boundary-check rewrite. Section 1 (world data model) → `rowDefAt`/`getOrCreateRow`/`ensureRowsGenerated`/`generateCarsForRow`. Section 2 (camera) → shared tween driving `scrollWorldRow`, fixed-anchor frog draw position. Section 3 (goal flow) → `onReachGoal` with teleport/invincibility-grant removed. Section 4 (hit/knockback) → collision handler's `KNOCKBACK_ROWS` clamp logic. Section 5 (cleanup) → `ensureRowsGenerated`'s deletion loop. All covered.
- **Cleanup/boundary direction check:** the spec's prose in section 5 describes the cleanup condition slightly ambiguously ("behind the frog" vs. the literal inequality it states). This plan resolves it via first-principles derivation from the camera formula in section 2 (verified against the original static board's exact row-0-at-top-at-spawn layout) rather than the possibly-inverted literal wording — rows are discarded once `worldRow > (visible bottom edge) + CLEANUP_MARGIN`, i.e. once they've scrolled past the bottom of the screen, which is what "behind the frog" means given the frog is anchored near the bottom and forward motion decreases `worldRow`.
- **Cosmetic note:** the original board's distinct "start" row (dark green, row 6) becomes a `'goal'`-typed row (cyan) under the periodic model, since `worldRow=6` and `worldRow=0` are both multiples of `GOAL_SPACING`. This is a minor, intentional simplification consistent with the spec's periodic-goal model — flagged here rather than silently introduced.
